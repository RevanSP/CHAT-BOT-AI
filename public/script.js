const callGroqAPI = async (message, signal) => {
  ...
  try {
    ...
    // 🟢 server sudah mengembalikan { message: "..." }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  whoIsReiivBtn.addEventListener("click", async () => {
    ...
    avatarDiv.className =
      "avatar mt-8 mb-5 flex justify-center items-center w-full";

    const avatarRingDiv = document.createElement("div");
    avatarRingDiv.className =
      "ring-primary ring-offset-base-100 w-32 rounded-full ring ring-offset-2 flex justify-center items-center";

    const imgElement = document.createElement("img");
    imgElement.src = "https://avatars.githubusercontent.com/u/110446896?v=4";
...
const addMessage = (
  ...
  const languageRegex =
    /javascript|python|java|html|css|typescript|php|ruby|go|c\+\+|c#|sql|nosql|graphql|node.js|react|angular|vue|express|mongodb|mysql|postgresql|sqlite/i;
  ...
  const isReiivMessage = content.includes(
    "REIIV is the creator and master of this chatbot.",
  );
  ...
  if (isCodeSnippet && !isUser && !isThinking && !isFirstMessage) {
    ...
    codeContainer.className =
      "code-snippet bg-base-300 p-4 mt-3 mb-3 rounded-md overflow-x-auto text-sm font-mono";
...
addMessage(
  `How can I help you today ? I am programmed by &nbsp;<div class="badge rounded bg-base-300 inline-flex items-center py-3"> <i class="bi bi-patch-check-fill w-3 text-blue-600"></i>
 &nbsp;&nbsp;<strong>REIIV</strong></div>&nbsp; Currently/default, using model : ${modelSelect.value}`,
  false,
  true,
);
